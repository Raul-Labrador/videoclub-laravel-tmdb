...
@section('content')
  ...
  <div class="page-header">
    <h1 class="page-title">Añadir Película</h1>
  ...
  <div class="form-card">
    <form action="{{ route('pelicula.store') }}" method="POST" enctype="multipart/form-data"> 
        ...
        <div class="form-group">
            ...
            <input class="form-control" minlength="3" maxlength="60" required id="titulo" name="titulo" placeholder="Título de la película" value="{{ old('titulo') }}" type="text">
            @error('titulo') <div class="form-error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" for="director">Director</label>
            <input class="form-control" minlength="3" maxlength="60" required id="director" name="director" placeholder="Director de la película" value="{{ old('director') }}" type="text">
            @error('director') <div class="form-error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" for="genero">Género</label>
            <input class="form-control" minlength="3" maxlength="60" required id="genero" name="genero" placeholder="Género de la película" value="{{ old('genero') }}" type="text">
            @error('genero') <div class="form-error">{{ $message }}</div> @enderror
        </div>

        <div class="form-group">
            <label class="form-label" for="actores">Actores</label>
            <textarea class="form-control" minlength="20" required id="actores" name="actores" placeholder="Actores que aparecen en la película" rows="4">{{ old('actores') }}</textarea>
        ...
        <div class="form-group">
            ...
            <input class="form-control" min="1" max="999" required id="duracion" name="duracion" placeholder="Ej: 120" value="{{ old('duracion') }}" type="number">
        ...
        <div class="form-group">
            ...
            <select class="form-control" required id="clasificacion" name="clasificacion">
                ...
                <option value="Apta para todos los publicos" @if(old('clasificacion') == 'Apta para todos los publicos') selected @endif>Apta para todos los públicos</option>
        ...
        <div class="form-group">
            <label class="form-label">Portada de la película <span style="color:var(--muted); font-weight:400;">(opcional)</span></label>
        ...
        <div class="form-actions">
            <button type="submit" class="btn btn-gold">Registrar Película</button>
